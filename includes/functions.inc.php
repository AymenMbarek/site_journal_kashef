...
<?php
  ...
  function createArticleCard($title, $img, $data, $category, $cat_id, $id, $color, $new, $trend, $marked) {
    ...
    if($trend) {
      echo '  <div class="tag tag-trend"><a href="search.php?trending=1">آخر الأخبار</a></div>';
    }
    echo '
        ...
        <a href="news.php?id='.$id.'" class="btn btn-card">اقرأ أكثر <span>&twoheadrightarrow; </span>
        </a>
      </div>
    </article>';
  }
  ...
  function createSlider($active, $img, $color, $category, $title, $content, $id, $new, $trend) {
    ...
    <div class="slide '.$active.'">
      ...
      <div class="info">
        ...
        if($trend) {
          echo '  <div class="tag tag-trend">آخر الأخبار</div>';
        }
      echo
        '
        <h1>'.$title.'</h1>
        ...
        <a href="news.php?id='.$id.'" class="btn btn-primary">اقرأ أكثر</a>
      </div>
    </div>';
  }
